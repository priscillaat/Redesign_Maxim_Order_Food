...
function showPage(pageId) {
    if(pageId !== 'page-notifikasi' && pageId !== 'page-peta' && pageId !== 'page-login' && pageId !== 'page-otp' && pageId !== 'page-activity' && pageId !== 'page-profil' && pageId !== 'page-bantuan' && pageId !== 'page-history') {
...
function goToPromo(viewOnly = false) { 
    isPromoViewOnly = viewOnly;
...
function renderPromos() {
    ...
    allPromos.forEach(p => {
        ...
        let onClickHTML = isPromoViewOnly ? '' : `onclick="applyPromoSelection('${p.id}', ${p.discount})"`;
        let cursorStyle = isPromoViewOnly ? 'cursor: default;' : 'cursor: pointer;';
...
function applyPromoSelection(code, discountValue) {
    ...
    showToast(`✔️ Promo ${code} berhasil dipasang!`); 
}
...
function goToTracking() {
    ...
    let finalTotalStr = document.getElementById('co-total-bayar').innerText.replace(/[^0-9]/g, '');
    let now = new Date();
    let monthNames = ["Jan", "Feb", "Mar", "Apr", "Mei", "Jun", "Jul", "Ags", "Sep", "Okt", "Nov", "Des"];
    let dateStr = `${now.getDate()} ${monthNames[now.getMonth()]} ${now.getFullYear()}, ${String(now.getHours()).padStart(2, '0')}.${String(now.getMinutes()).padStart(2, '0')} WIB`;
    let orderId = `MAXM - ${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')} - ${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`;

    currentOrderDetails = {
        restoName: currentCheckoutResto,
        imgResto: resto.imgResto,
        firstFoodImg: resto.imgFood,
        firstFoodName: resto.food,
        firstFoodPrice: resto.price,
        total: finalTotalStr,
        date: dateStr,
        orderId: orderId
    };
...
function batalPesanan() {
    ...
    if(konfirmasi) {
        if (currentOrderDetails) {
            currentOrderDetails.status = 'Dibatalkan';
            orderHistory.unshift(currentOrderDetails);
...
function rateResto(num) {
    ...
    setTimeout(() => { 
        ...
        if (currentOrderDetails) {
            currentOrderDetails.status = 'Selesai';
            orderHistory.unshift(currentOrderDetails);
            currentOrderDetails = null;
        }
...
// REVISI: Fungsi baru untuk membersihkan seluruh riwayat pesanan
function clearHistory() {
    if (orderHistory.length === 0) return;
    if (confirm("Apakah Anda yakin ingin menghapus semua riwayat pesanan Anda?")) {
        orderHistory = [];
        renderHistory();
        showToast("🗑️ Semua riwayat pesanan telah dihapus");
    }
}
...
function renderHistory() {
    ...
    if (orderHistory.length === 0) {
        container.innerHTML = '<p style="text-align:center; color:#888; margin-top:20px;">Belum ada histori pesanan.</p>';
        return;
    }

    orderHistory.forEach(order => {
        let statusColor = order.status === 'Selesai' ? '#00ff00' : '#ff4444';
        
        container.innerHTML += `
        <div class="history-card">
            <div class="history-card-header">
                <img src="${order.imgResto}" class="history-resto-img" alt="Resto">
                <div class="history-resto-info">
                    <h3 style="color: #ffeb00; font-size: 15px; margin: 0;">${order.restoName}</h3>
                </div>
                <div style="color: ${statusColor}; font-size: 14px; font-weight: bold; margin-left: auto;">${order.status}</div>
            </div>
            
            <div style="margin-top: 15px; margin-bottom: 20px; font-size: 13px; color: #fff; line-height: 1.5;">
                <div>Pesanan tgl : ${order.date}</div>
                <div>ID : ${order.orderId}</div>
            </div>

            <div style="display: flex; gap: 15px; align-items: center;">
                <img src="${order.firstFoodImg}" style="width: 60px; height: 60px; border-radius: 8px; object-fit: cover;">
                <div>
                    <div style="font-weight: bold; font-size: 14px; color: #fff;">${order.firstFoodName}</div>
                    <div style="font-size: 13px; color: #fff; margin-bottom: 5px;">Rp.${parseInt(order.firstFoodPrice).toLocaleString('id-ID')}</div>
                    <div style="font-weight: bold; font-size: 14px; color: #fff;">Total : ${parseInt(order.total).toLocaleString('id-ID')}</div>
                </div>
            </div>

            <div style="text-align: right; margin-top: 20px;">
                <span style="color: #fff; font-weight: bold; font-size: 14px; cursor: pointer;" onclick="goToHome()">Pesan Lagi >></span>
            </div>
        </div>
        `;
    });
...
let simPhoneInterval;

function openSimPhone() {
    document.getElementById('sim-phone-popup').style.display = 'flex';
    let timerEl = document.getElementById('sim-phone-timer');
    timerEl.innerText = "00:00";
    let seconds = 0;
    clearInterval(simPhoneInterval);
    simPhoneInterval = setInterval(() => {
        seconds++;
...
function closeSimPhone() {
    clearInterval(simPhoneInterval);
    document.getElementById('sim-phone-popup').style.display = 'none';
}

function openSimChat() {
    document.getElementById('sim-chat-popup').style.display = 'flex';
    let chatBody = document.getElementById('sim-chat-body');
    chatBody.scrollTop = chatBody.scrollHeight;
}

function closeSimChat() {
    document.getElementById('sim-chat-popup').style.display = 'none';
}

function sendSimChat() {
    let input = document.getElementById('sim-chat-input');
    ...
    userDiv.style.cssText = "background: #ffeb00; color: #000; padding: 10px 15px; border-radius: 15px; border-bottom-right-radius: 5px; align-self: flex-end; font-size: 13px; max-width: 80%; font-weight: bold;";
    ...
    if (lowerMsg.includes("sambal") || lowerMsg.includes("sambel") || lowerMsg.includes("pedas") || lowerMsg.includes("pedes")) {
        driverResponse = "Siap, nanti saya sampaikan ke resto ya, Kak.";
    } else if (lowerMsg.includes("posisi") || lowerMsg.includes("dimana") || lowerMsg.includes("di mana")) {
        driverResponse = "Ini sedang menunggu makanan dibuatkan di resto, Kak.";
    } else if (lowerMsg.includes("halo") || lowerMsg.includes("p") || lowerMsg.includes("siang") || lowerMsg.includes("sore") || lowerMsg.includes("malam")) {
        driverResponse = "Halo Kak, ada yang bisa saya bantu terkait pesanannya?";
    } else if (lowerMsg.includes("makasih") || lowerMsg.includes("terima kasih")) {
        driverResponse = "Sama-sama Kak, mohon ditunggu ya, langsung saya antar jika sudah ready.";
    } else if (lowerMsg.includes("sesuai aplikasi") || lowerMsg.includes("sesuai apk")) {
        driverResponse = "Oke siap Kak, laksanakan!";
    } else {
        let defaultResponses = [
            "Baik Kak, mohon ditunggu sebentar ya.",
            "Siap Kak, pesanan sedang diproses.",
            "Oke Kak, laksanakan. Ini saya pantau terus restonya."
        ];
        let randomIndex = Math.floor(Math.random() * defaultResponses.length);
        driverResponse = defaultResponses[randomIndex];
    }
    
    setTimeout(() => {
        let driverDiv = document.createElement('div');
        driverDiv.style.cssText = "background: #222; color: white; padding: 10px 15px; border-radius: 15px; border-bottom-left-radius: 5px; align-self: flex-start; font-size: 13px; max-width: 80%;";
        driverDiv.innerText = driverResponse;
        chatBody.appendChild(driverDiv);
        chatBody.scrollTop = chatBody.scrollHeight;
    }, 1500);
}